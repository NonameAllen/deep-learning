...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, channel, block_num, stride=1):
        downsample = None
        # ------------------捷径分支是否需要downsample: 宽高压缩 或者 改变通道来判断
        ...
        # conv2_x,conv3_x,conv4_x,conv5_x 一系列残差结构的第一层，只有第一层存在虚线残差结构，由于需要调整宽高或通道
        layers = []
        layers.append(block(self.in_channel, channel, downsample=downsample, stride=stride))
        self.in_channel = channel * block.expansion

        # conv2_x,conv3_x,conv4_x,conv5_x除了第一层外，其他层都是实线残差结构，可以直接堆叠
...
def train_main():
    ...
    print(class_names)
    # resnet 自定义
    net = resnet34(len(class_names))
    # inchannel = net.fc.in_features
    # net.fc = nn.Linear(inchannel,len(class_names))
    net.to(device)
   # print(net)
...
# todo num_epochs 25
    model_bset = train_model(net, dataloaders, dataset_sizes, criterion, optimizer_ft, exp_lr_scheduler,num_epochs=num_epochs)

    torch.save(model_bset, "resnet_trashv1_4.pt")
    pltLoss()
